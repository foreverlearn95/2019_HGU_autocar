...
/*	
	...
	corners[0]=Point2f(0,YHalf);
	corners[1]=Point2f(Width,YHalf);
	corners[2]=Point2f(0,Height);
	corners[3]=Point2f(Width,Height);
	Size warpSize(320,120);
	...
	warpPerspective(frame,warpimg,trans,warpSize);
	for(int i=0;i<corners.size();i++)
		circle(frame,corners[i],3,Scalar(0,255,0),3);
	cvtColor(warpimg,roi,COLOR_BGR2GRAY);
*/
    cvtColor(frame, grayframe, COLOR_BGR2GRAY);
    Rect rect_roi=Rect(0,YHalf,Width,YHalf);
    roi = frame(rect_roi);
    ...
    Canny(grayframe,edge_frame,50,150,3); //min_val, max val , filter size 70 150 3

    vector<cv::Vec4i> lines_set;

    cv::HoughLinesP(edge_frame,lines_set,1,PI/180,20,20,10); //30 30 10
