...
const int YPoint = 40; //best 50

float slope_threshold = 0.5; //best 0.6
...
clock_t start = clock(), ending; //ennnnnnnnnnnd
...
int check_point_detection(Mat & a) {
   ...
   for (int i = 60; i < 240; i++) {
      ...
      white_point = gray.at<uchar>(120, i); //200 -> 120
...
//pixel_count>60
      ...
      double interval = (double)(ending - start) / CLOCKS_PER_SEC;
...
//	  cout << differ << endl;
   ...
   // y = mx + b ==> x0 = (y0-b)/m
  // l-r = (XHalf - ((YPoint - left_b_mem) / left_slope_mem)) - (((YPoint - right_b_mem) / right_slope_mem) - XHalf)
...
if(*rdistance>200)
    *rdistance=200;
else if(*rdistance<-200)
    *rdistance=-200;

if(*ldistance>200)
    *ldistance=200;
else if(*ldistance<-200)
    *ldistance=-200;
*/
if(*rdistance>200)
...
*/
/*
	else if(differ >150 && differ<200)
		differ=200;
	else if(differ<-150 && differ >-200)
		differ=-200;
	else if(differ >100 && differ < 150)
		differ=150;
	else if(differ<-100 && differ >-150)
		differ=-150;
	else if(differ>50 && differ <100)
		differ=100;
	else if(differ<-50 && differ >-100)
		differ=-100;
*/
	else if(differ<17 && differ >-17)
		differ=0;
   return differ;
...
//Preparing to capture video from here
...
writer.open("capture.avi",VideoWriter::fourcc('M','J','P','G'),fps,size,true);
...
//to here
  ...
  ros::Publisher pub3 = nh3.advertise<std_msgs::Int16>("stop_msg", 100);
...
//lab_count and return

      check_point_detection(frame);


      cout << "count : " << lab_count <<endl;
...

      {
		  ...
		  stop_msg.data = lab_count;

		  pub3.publish(stop_msg);
